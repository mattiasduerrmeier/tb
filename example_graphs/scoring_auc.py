...
def main():
    ...
    parser.add_argument("--all", default=False, action='store_true',
                        help='The embeddings are evaluated on all training percents from 10 to 90 when this flag is '
                             'set to true. '
    ...
    for train_percent in training_percents:
        for shuf in shuffles:
            ...
            one_hot = mlb.fit_transform(y_test)
            for x in one_hot:
                if max(x) == 0:
                    __import__('pdb').set_trace()

            # for average in averages:
            results = roc_auc_score(mlb.fit_transform(y_test), preds, average=None)
